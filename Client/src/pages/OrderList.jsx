...
const columns = [
    ...
    },
    {
        ...
        dataIndex: "quantity",
        sorter: (a, b) => a.quantity.toLocaleString(b.quantity) || b.quantity.toLocaleString(a.quantity),
    },
    {
        title: "Total Amount",
        dataIndex: "amount",
...
const OrderList = () => {
    ...
    for (let i = 0; i < orderState?.length; i++) {
        ...
        data1.push({
            ...
            product: <Link className='viewColor' to={``}>View Item</Link>,
            status: orderState[i]?.delivery_status,
            action: (
                <>
                    <Link to={`/checkout-success/${userId}/${transitionId}`} className='ms-3'>
