...
const updateCategory = async (updatedData) => {
    try {
        const response = await api.put(`/category/update/${updatedData.id}`, {title: updatedData.CategoryData.title});
        return response.data;
    } catch (error) {
        console.error("An error occurred while creating category:", error.message);
        throw new Error(); // Re-throw the error for higher-level handling
...
const getCategory = async (id) => {
    try {
        ...
        console.error("An error occurred while fetching single category with id:", error.message);
        throw new Error(); // Re-throw the error for higher-level handling
    }
}
