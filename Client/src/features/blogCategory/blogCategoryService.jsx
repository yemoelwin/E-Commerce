...
const allBlogCategory = async () => {
    try {
        ...
        console.error("An error occurred while fetching all blogCategories:", error);
...
const updateBlogCategory = async (data) => {
    try {
        const response = await api.put(`/blogCategory/update/${data.id}`, { title: data.blgCatgoryData.title });
        return response.data;
    } catch (error) {
...
const BlogCategory = async (id) => {
    try {
        ...
        console.error("An error occurred while fetching single blog category", error);
        throw error; // Re-throw the error for higher-level handling
    }
};
