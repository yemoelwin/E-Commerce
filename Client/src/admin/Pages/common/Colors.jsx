...
const columns = [
	...
	},
	{
		...
		dataIndex: "name",
		sorter: (a, b) =>
			a.color.localeCompare(b.color) || b.name.localeCompare(a.name),
	},
	{
		title: "Total Views",
		dataIndex: "views",
...
const Colors = () => {
	...
	const sortedColorState = colorState?.slice().sort((a, b) => {
	...
	for (let i = 0; i < sortedColorState.length; i++) {
		data1.push({
			key: i + 1,
			name: sortedColorState[i].color,
			views: sortedColorState[i].numSearchs,
			action: (
				<>
					<Link
						to={`/admin/edit-color/${sortedColorState[i]._id}`}
						className=''
					>
						Edit
					</Link>
					<button
						onClick={() => showModal(sortedColorState[i]._id)}
