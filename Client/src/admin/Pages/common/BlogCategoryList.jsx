...
const BlogCategoryList = () => {
	...
	const sortedBlogCategoryState = blogCategoryState?.slice().sort((a, b) => {
	...
	for (let i = 0; i < sortedBlogCategoryState?.length; i++) {
		data1.push({
			key: i + 1,
			title: sortedBlogCategoryState[i]?.title,
			views: sortedBlogCategoryState[i]?.numSearchs,
			action: (
				<>
					<Link
						to={`/admin/edit-blog-category/${sortedBlogCategoryState[i]?._id}`}
						className=''
					>
						Edit
					</Link>
					<button
						...
						onClick={() => showModal(sortedBlogCategoryState[i]?._id)}
					>
