...
export const stripeSession = async (req, res) => {
    ...
    // console.log('cartItemData', cartItemData, transitionId)
...
export const webHook = async (req, res) => {
    ...
    if (eventType === 'checkout.session.completed') {
        try {
            ...
            const order = await orderModel.find({ transitionId: transitionId, stripe_response: false })
            console.log("order", order)
            console.log('orderProductId0', order[0].products[0].productId)
                ...
                for (const productObj of productsArray) {
                    ...
                    if (productObj && productObj.productId) {
                        ...
                        if (prod && prod.length > 0) {
                            ...
                            prod[0].quantity = Math.max(0, prod[0].quantity - productObj.quantity);
